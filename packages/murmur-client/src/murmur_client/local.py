"""``LocalClient`` — in-process counterpart to :class:`MurmurClient`.

Same calling surface as :class:`murmur_client.MurmurClient`, but
dispatches against an in-process :class:`murmur.AgentRuntime` instead
of HTTP. Use this when both producer and consumer live in the same ASGI
process (e.g. a FastAPI app with :class:`murmur.server.AgentRouter`
mounted) and you want to skip the httpx round-trip.
...
class LocalClient:
    """Async in-process client. Same surface as :class:`MurmurClient`.
    ...
    async def close(self) -> None:
        """No-op for the local client — kept for API symmetry with
        :class:`MurmurClient`. The backing :class:`AgentServer` and its
        runtime own their own lifecycles; we don't tear them down here.
        """
        return None
    ...
    async def submit(
        ...
        *,
        ...
        return Run(client=self, run_id=run_id, target=target)
